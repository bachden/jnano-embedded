...
public class inproc_lat {
	static class Worker implements Runnable {
		public Worker(NanoLibrary nano, int roundtrip_count, int message_size, String addr, CountDownLatch done) {
			this.nano = nano;
		...
		private void do_run() throws InterruptedException {
			int socket;
			int rc;
			int i;
...
//		}
		...
		message_size = 1024;// Integer.parseInt(args[0]);
		roundtrip_count = (int) 1e6;// Integer.parseInt(args[1]);
		...
		new Thread(new Worker(nano, roundtrip_count, message_size, addr, done)).start();
